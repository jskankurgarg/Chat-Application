...
public class RMIClient extends JFrame implements ActionListener, Runnable
{
    ...
    private void initComponents()
    {
        ...
        jButtonConnect.setFont(new java.awt.Font("Trebuchet MS", 0, 11));
        ...
        jButtonSend.setFont(new java.awt.Font("Trebuchet MS", 0, 11));
        ...
        jLabelUserList.setFont(new java.awt.Font("Trebuchet MS", 0, 11));
    ...
    public void run()
    {
        System.out.println(socket);//in cmd prompt
        try
        {
            in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
//reader is wrapper class for streams 
    ...
    public void actionPerformed(ActionEvent ae)
    {
        if (ae.getActionCommand().equals("Connect") && !jTextUserName.getText().equals(""))
        {
            try
            {
                ...
                thread.start();// chatting started that is receiving module has been activated
				// run method execution begins
        ...
        else if (ae.getActionCommand().equals("Disconnect"))
        {
            try
            {
                ...
                listClient.clear();
                jButtonConnect.setText("Connect");
                thread.interrupt();// again application has to be restarted
            }
            catch (RemoteException re) {}
        }
        else if (ae.getSource() == jButtonSend && !jTextSendMessage.getText().equals(""))//since connect button object is same we caanot use this 
		//comparison in above if-else-if construct
        {
            try
            {
